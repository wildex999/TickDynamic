...
public class TimedTileEntities extends TimedGroup {
    ...
    public void initGroup(String configEntry) {
		...
		if(configEntry != null)
		{
			setSliceMax(mod.config.get(configEntry + ".tileentity", configKeySlicesMax, mod.defaultTileEntitySlicesMax).getInt());
			setMinimumObjects(mod.config.get(configEntry + ".tileentity", configKeyMinimumObjects, mod.defaultTileEntityMinimumObjects).getInt());
		}
		else
		{
	...
	public int startUpdateObjects(World world) {
		...
		System.out.println(name + ": ticking " + remainingObjects + " of " + loadedList.size());
